...
namespace Engine.ComputerGraphics.Component
{
    public class IslandBehaviour : MonoBehaviour
    {
        ...
        [Range(0.1f, 0.9f)]
        public float PercentageWater = 0.2f;
        ...
        private Dictionary<int, BackupTerrain> terrainBackupDictionary = new Dictionary<int, BackupTerrain>();

        public void SetFlatTerrainAround(GameObject obj)
        {
            BackupTerrain backupTerrain = null;
            ...
            if (Physics.Raycast(ray, out hit, 100))
            {
                ...
                if (hit.collider.gameObject.name.Equals("Terrain"))
                {
                    ...
                    try
                    {
                        ...
                        //get backup terrain if exists
                        if (terrainBackupDictionary.ContainsKey(obj.GetHashCode()))
                        {
                            backupTerrain = terrainBackupDictionary[obj.GetHashCode()];
                            terrainBackupDictionary[obj.GetHashCode()] = new BackupTerrain(obj, heights, startx, startz, width, height);
                        }
                        else
                        {
                            BackupTerrain bt = new BackupTerrain(obj, heights, startx, startz, width, height);
                            terrainBackupDictionary.Add(obj.GetHashCode(), bt);
                        }
                    }
                    catch (Exception e)
                    {
                    ...
                    {
                        terrain.terrainData.SetHeights(backupTerrain.startX, backupTerrain.startZ, backupTerrain.terrainData);
                        Debug.Log("backup");
                    }
