...
public class CustomUIDragDropItem : MonoBehaviour
{
    //Edit
    private float startPosition_Y = 0;
    public GameObject GridCharacters;
    private static ViewCharacter characterDragged;
...
#region Common functionality
	...
	protected virtual void StartDragging ()
	{
		if (!mDragging)
		{
			if (cloneOnDrag)
			{
                ...
                characterDragged = gameObject.GetComponent<ViewCharacter>();
                ...
                CustomUIDragDropItem item = clone.GetComponent<CustomUIDragDropItem>();
				item.mDragging = true;
				item.Start();
	...
	protected virtual void OnDragDropStart ()
	{
        ...
        startPosition_Y = gameObject.transform.localPosition.y;
	...
	{
        //if (!cloneOnDrag || cloneOnDrag) //Fix
        //{
        ...
        this.gameObject.GetComponent<ViewCharacter>().Character = characterDragged.Character;

        bool error = ViewCombineCharacter.Elaborate(surface, this.gameObject, cloneOnDrag);

        if ( cloneOnDrag )
        {
            if (error) Destroy(gameObject);
        }
        else
        {
            if (error) Revert();
    ...
    private void Revert()
    {
        //GameObject grid = GameObject.Find("ContainerScrollView");
        UIDragDropContainer gridContainer = 
            NGUITools.FindInParents<UIDragDropContainer>(GridCharacters);

        //: container.transform;
        mTrans.parent = gridContainer.reparentTarget;
        mTrans.localPosition = new Vector3(50,startPosition_Y + 25,0);
